...
export class FacebookService {
  ...
  init(){
    ...
    //console.dir(this.platformLocation);

    this.location = window.location;//this.platformLocation.getBaseHrefFromDOM();
    console.dir(this.location);

console.log('via Search:');
console.dir(this.location.search);
console.dir(window.location.search);
    ...
    if(!this.access_token){
      console.log('via Hash:');
      console.dir(this.location.hash);
  ...
  get_token(){
    ...
    if(!(this.access_token)){
      ...
      // with Observable
      /*this.http.get(this.cnf_url).subscribe(
        (resp: Response) => this.fetch_token(resp),
        (ex:any) => this.handleOAuthError(ex)
      );*/

      //with Promice
      /*this.http.get(this.cnf_url).toPromise()
      .then(
        (resp:Response) => this.getObjects(resp)
      )
      .catch(
        (ex:any) => this.handleOAuthError(ex)
      );*/
